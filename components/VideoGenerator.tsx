...
const VideoGenerator: React.FC = () => {
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      // Calls generateVideoVeo from geminiService
      const url = await generateVideoVeo(prompt, sourceImage, aspect, setProgressMsg);
      setVideoUrl(url);
    } catch (err: any) {
      console.error(err);
      // Specific handling for mandatory API key selection issues
      if (err.message === "API_KEY_REQUIRED") {
        setProgressMsg("API Key re-selection required...");
        await window.aistudio.openSelectKey();
        setProgressMsg("API Key updated. Please try again.");
      } else {
        setProgressMsg("Error: Cinematic generation requires a paid billing account.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <header className="hidden md:block">
        ...
        <div className="flex items-center gap-3">
           ...
           <span className="bg-yellow-500/10 text-yellow-500 text-[8px] px-2 py-0.5 rounded border border-yellow-500/20 font-black uppercase tracking-widest">Paid Key Required</span>
        </div>
      </header>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                <p className="text-[9px] text-zinc-600 text-center uppercase tracking-tighter">
                  Video features require a paid key selection from ai.google.dev
                </p>
              </div>
            </div>
          </div>
