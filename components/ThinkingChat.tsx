...
const ThinkingChat: React.FC = () => {
  ...
  // Sync current conversation to messages
  useEffect(() => {
    if (currentId) {
  ...
  const handleSend = async () => {
    ...
    // Create ID if this is a new chat
    ...
    try {
      // Pass the existing message history to maintain context
      const response = await thinkingChat(input, messages);
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-[500px] md:min-h-0 space-y-4">
        <header className="flex-shrink-0 hidden md:flex justify-between items-end">
          <div>
            ...
            <p className="text-zinc-500 text-xs tracking-wide">Gemini 3 Pro • Thinking Enabled</p>
          </div>
        </header>
        ...
        >
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center opacity-20 px-8">
              ...
              <p className="max-w-xs text-xs font-medium leading-relaxed">System ready for complex multi-step reasoning and deep analysis.</p>
            </div>
          )}
