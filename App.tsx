...
const App: React.FC = () => {
  ...
  const handleNavClick = (mode: AppMode) => {
    // All modes are now optimized for Free Tier access
    setActiveMode(mode);
  };
  ...
  return (
    <div className="min-h-[100dvh] bg-black text-white flex flex-col md:flex-row overflow-hidden">
      ...
      <nav className="hidden md:flex w-64 border-r border-zinc-800 p-4 flex-col gap-2 h-screen sticky top-0 bg-black z-50">
        ...
        <div className="mt-auto p-4 bg-zinc-900/30 rounded-2xl border border-zinc-800/50 text-[10px] text-zinc-600 uppercase tracking-widest text-center flex flex-col gap-2">
          <p>Optimized for Gemini Free Tier</p>
          <button 
            onClick={() => window.aistudio.openSelectKey()}
            className="text-[8px] text-zinc-500 hover:text-zinc-400 underline"
          >
            Manage API Access
          </button>
        </div>
      ...
      <header className="md:hidden flex items-center justify-between p-4 border-b border-zinc-900 bg-black/80 backdrop-blur-md sticky top-0 z-50">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-bold tracking-tighter">AI Jozef ®</h1>
        </div>
      ...
      {/* Main Content Area */}
      <main className="flex-1 overflow-y-auto h-full flex flex-col pb-24 md:pb-0">
        ...
        {/* Modern Footer with Instagram Link */}
